...
def monteHall(l):
    for j in range(26):
        ...
        for i in range(trials):
            ...
            #randomOrder.remove(randomOrder[i])
            randomOrder[j] = None
            newl = getNewL(randomOrder)
            othercase = newl[0]
            if mycase > othercase:
                score += 1
        print("If you always pick case ", j+1, " to start, its probability of having more money than the last case on stage is ", score/trials)
    return 0
